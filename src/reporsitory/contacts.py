...
def update_user_email_verification(db: Session, token: str):
...
def update_user_avatar(db: Session, user_id: int, avatar: UploadFile):
    user = db.query(User).get(user_id)
    
    # Завантажуємо аватар на Cloudinary
    result = cloudinary.uploader.upload(avatar.file)
